...
if(isset($_GET['req_id'])){
    ...
    $req_id=$_GET['req_id'];
    $p_id=$_GET['p_id'];
    $p_seats=$_GET['p_seats'];
    $s_id=$_GET['s_id'];
    $aprove="Approved";

    $query_sel = "SELECT * from `post`  WHERE `p_id`='$p_id'";
    $result_sel = $admin->ret($query_sel);
    $row_post = $result_sel->fetch(PDO::FETCH_ASSOC);
    $current_seats=$row_post['p_seats'];
    $current_seats--;

    $query_up = "UPDATE `post` SET `p_seats`='$current_seats' WHERE `p_id`='$p_id' ;";
    $result_up = $admin->cud($query_up, "updation Successful");
    $query = "UPDATE `request` SET `req_requst`='$aprove' WHERE `req_id`='$req_id' ;";
    $result = $admin->cud($query, "updation Successful");

    $out_stu = $admin->ret("SELECT * FROM `student` WHERE `s_id`='$s_id' ;");
    $Stu = $out_stu->fetch(PDO::FETCH_ASSOC);
    $student_email=$Stu['s_email_id'];
    $student_Name=$Stu['s_name'];
    // $student_email;
    $subject="Approoved";
    $mes='<!DOCTYPE html>
    <html lang="en">
        ...
        <div style="background-color: #d6e7f0; width: 50%; height: fit-content; margin-top: 60px; margin-bottom: 20px; text-align: center; margin: auto;border-top-left-radius: 14px;
            ...
            <div style="background-color: #d6e7f0;">
                ...
                <h1 style="
                                                                                  ...
                                                                                  color: #052d3d;
                                                                                  line-height: 1.5;">'.$student_Name.' Your Application form Has been Accepted</h1>
                <h1 style=" 
                                                                                  font-family:  Lato, sans-serif;
                    mso-line-height-alt: 18px; color: red; font-size: 65px; line-height: 1.5; "></h1>
    ...
    $mail = $admin->mail_send($subject,$student_email,$mes);

 //opens file in append mode
 $t=time();
 $text="\n".date("Y-m-d",$t)."  ".$req_id."  ".$p_id."  ".$aprove."  ".$p_seats."   "." -Centre approoves request of the student";
 $fp = fopen('../../log.txt', 'a');  
 fwrite($fp, $text);  
