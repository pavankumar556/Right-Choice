...
if(isset($_POST['uname'])){
    ...
    $query_check = $admin->ret("SELECT * FROM `student` WHERE `s_email_id`='$username'");
   ...
   if($confirm_password==$new_password){
        ...
        $query = "UPDATE `student` SET `s_password`='$confirm_password' WHERE `s_email_id`='$email';";
        $result = $admin->cud($query, "updation Successful");


    //opens file in append mode
    $t=time();
    $text="\n".date("Y-m-d",$t)."  ".$email."  ".$new_password."  ".$result."   "." -Forgot  student";
    $fp = fopen('../log.txt', 'a');  
    fwrite($fp, $text);  
